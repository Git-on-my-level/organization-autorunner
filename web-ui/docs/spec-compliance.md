...
| Requirement                                                                             | Status | Implementation                                                                                                                                                                            | Known limitations                                                                           | Tests                                                                                                                                        |
| --------------------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.1 Treat oar-core as source of truth                                                   | Met    | All reads/writes via `coreClient`/`oarCoreClient`; `src/hooks.server.js` resolves proxied traffic from the active `/:project/...` context and `OAR_PROJECTS` catalog                      | No offline cache invalidation strategy beyond fresh fetches                                 | `tests/e2e/integration-core-golden-path.spec.js`                                                                                             |
...
| 1.2 Support board primitives and workspace shape                                        | Met    | Board list/detail pages use `boards.list`, `boards.get`, and `boards.workspace` through `src/lib/oarCoreClient.js`; board detail reads the server-owned canonical workspace projection | No drag-and-drop ordering in v1 by design                                                   | `tests/e2e/boards.spec.js`                                                                                                                    |
...
| 1.4 Actor identity required for writes                                                  | Met    | Actor gate + project switcher in `src/routes/+layout.svelte`; project-scoped actor/auth state in `src/lib/actorSession.js` and `src/lib/authSession.js`; `withActorId` in `oarCoreClient` | Actor selection remains browser-local, but isolated per project                             | `tests/e2e/shell.spec.js`, `tests/unit/actorSession.test.js`, `tests/unit/actorClient.integration.test.js`                                   |
...
| Thread detail shows snapshot + timeline distinction                            | Met    | Snapshot section and timeline section in `src/routes/[project]/threads/[threadId]/+page.svelte`  | None                         | `tests/e2e/thread-detail.spec.js`                                      |
...
| Requirement                                                   | Status | Implementation                                                                       | Known limitations                                                                   | Tests                                                                                                                          |
| ------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Inbox grouping, navigation, ack, decision recording           | Met    | `src/routes/[project]/inbox/+page.svelte`, grouping utils in `src/lib/inboxUtils.js` | No ranking engine (expected v0 behavior)                                            | `tests/e2e/inbox.spec.js`, `tests/unit/inboxUtils.test.js`                                                                     |
| Thread list with filters and creation                         | Met    | `src/routes/[project]/threads/+page.svelte`, `src/lib/threadFilters.js`              | No server-side pagination yet                                                       | `tests/e2e/threads.spec.js`, `tests/unit/threadFilters.test.js`                                                                |
| Thread detail edit + commitments + timeline + message posting | Met    | `src/routes/[project]/threads/[threadId]/+page.svelte`                               | Artifact linking is via editable typed refs in `key_artifacts`, no dedicated picker | `tests/e2e/thread-detail.spec.js`, `tests/e2e/commitments.spec.js`                                                             |
| Board list and board detail mutation flows                   | Met    | `src/routes/[project]/boards/+page.svelte`, `src/routes/[project]/boards/[boardId]/+page.svelte`                                         | Explicit controls only; no drag-and-drop in v1                                             | `tests/e2e/boards.spec.js`                                                                                                                    |
...
| Optimistic locking on board and board-card edits | Met  | Board create/edit/card add-move-update-remove flows send `if_updated_at` / `if_board_updated_at`; conflict responses trigger a recoverable reload message and fresh workspace fetch | No merge UI for stale board forms                                                  | `tests/e2e/boards.spec.js`                                                                                                                    |
...
| Parse and navigate `board:` refs across surfaces     | Met    | Same typed-ref pipeline plus board routes under `src/routes/[project]/boards/*`; verified by `tests/unit/typedRefs.test.js`, `tests/unit/refLinkModel.test.js`, and `tests/e2e/boards.spec.js` |
