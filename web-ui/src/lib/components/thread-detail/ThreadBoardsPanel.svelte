<script>
  ...
  function statusTone(status) {
    ...
    if (status === "closed") return "text-[var(--ui-text-muted)] bg-[var(--ui-border)]";
  ...
  {:else}
    <div class="divide-y divide-[var(--ui-border-subtle)]">
      ...
      {#each boardMemberships as membership}
        ...
        {@const columnKey = membership?.card?.column_key ?? membership?.column_key}
        {#if boardId}
          <a
            class="flex items-center justify-between gap-3 px-4 py-2.5 transition-colors hover:bg-[var(--ui-bg-soft)]"
          ...
          >
            <div class="flex min-w-0 items-center gap-2">
              ...
              {#if boardStatus}
                <span
                  class="shrink-0 rounded px-1.5 py-0.5 text-[10px] font-semibold {statusTone(boardStatus)}"
                >
