...
var defaultErrorMetadata = map[string]errorMetadata{
	...
	"last_active_principal":      {Recoverable: true, Hint: "Retry with `allow_human_lockout=true` and a non-empty `human_lockout_reason` only for explicit break-glass recovery; it can leave the workspace inaccessible."},
