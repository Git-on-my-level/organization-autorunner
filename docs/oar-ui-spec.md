...
- oar-ui MUST support all primitives and typed conventions defined in `oar-schema.yaml`: events, snapshots, artifacts, threads, commitments, work orders, receipts, reviews.
...
- oar-ui MUST display provenance using the standardized provenance shape defined in `oar-schema.yaml` → `provenance`.
...
- oar-ui MUST follow the reference conventions defined in `oar-schema.yaml` → `reference_conventions` when creating events.
